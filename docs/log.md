...
## [2026-07-22] build | Desk polish — notes, customer context, typeahead, Verkilo rebrand
- **Notes** (`Notable` concern): one un-versioned `has_rich_text :note` mixed into `Record` (so Ticket/License notes ride the stable spine identity and survive version churn) and `Customer`. One polymorphic `NotesController#update` (`PATCH /note`); the target rides in the form as a signed, purpose-scoped GlobalID, admin-gated via the notable's own policy. `shared/_note` = collapsed `<details>` + Lexxy editor with a dull-yellow "sticky" background (`color-mix` off `--canvas` → one shade off in both themes; editor inherits `--lexxy-color-canvas`). Customer's old plain-text `notes` column retired from the UI (column left in place, unused — droppable later).
- **Customer context on a ticket**: "Also from <customer>" panel — their 3 most recent tickets or all in the last 30 days, whichever is larger; subject link + status + `relative_time` opened/updated (new helper: "3 days ago" within a week, exact date/time on hover).
- **New Ticket form**: full-width; customer picker is a `<datalist>` typeahead (`customer-picker` Stimulus resolves name → `customer_id`). **"Open & send to customer"** emails the opener (`TicketMailer#new_ticket`), stores the SES Message-ID on the ticket, sets pending.
- **Customers index**: key marker + `?licensed` filter for licensed (priority) customers.
- **Verkilo rebrand of all outbound email**: From "Verkilo Support"; ticket reply/opener sign "The Crew at Verkilo"; magic-link subjects/body say Verkilo. No "Covenant" reaches any inbox (web chrome unchanged — customers never see it). License placeholder → "Verkilo".
- pages touched: [[overview]], [[index]], [[support-desk-plan]], [[log]]
- refs: `app/models/concerns/notable.rb`, `app/controllers/notes_controller.rb`, `app/helpers/application_helper.rb`, `app/controllers/tickets_controller.rb`, `app/controllers/customers_controller.rb`

## [2026-07-22] build | Ticket workflow — status buttons, reply↔pending/open, auto-close, quoted-history collapse
- **Status controls** on Ticket#show: **Resolved** / **Hold** (subdued red) / **Reopen** in the head (Edit removed — the customer's opener is never editable: excluded from `update`, hidden on the edit form). "Tickets" breadcrumb + app-menu link return to the last filter (session), defaulting to Open.
- **Lifecycle automation**: an agent reply sets the ticket **pending**; a customer's inbound reply reopens a pending ticket to **open**; `resolved_at` stamped in `Ticket#build_successor`; `Ticket::CloseResolvedJob` (daily, `config/recurring.yml`) **closes** tickets resolved ≥ 1 week.
- **Inbound HTML cleanup**: strip `<style>`/`<script>` + the sender's presentational cruft (HEY ships Trix wrappers); **fold the quoted history into a collapsed `<details>`** (top-post-aware cut: "On … wrote:" attribution else last quote block, within its own parent so a wrapped body isn't wholly collapsed). `<details>`/`<summary>` allowlisted in Action Text (after Lexxy).
- refs: `app/mailboxes/tickets_mailbox.rb`, `app/models/ticket.rb`, `app/jobs/ticket/close_resolved_job.rb`, `config/recurring.yml`, `config/application.rb`

## [2026-07-22] build | SES live — deployed to covenant.verkilo.com; threading re-anchored on the SES Message-ID
- **Phase 0 complete + deployed.** Reused the sibling `merovex-press` AWS account (IAM `cohwall-ses`, already out of sandbox, account suppression on). New for verkilo.com: two DKIM identities (`verkilo.com` transactional apex, `news.verkilo.com` bulk provisioned-ahead), MAIL FROM `bounce.*` (MX+SPF), DMARC softened to `p=quarantine`+`rua` for bring-up. Inbound: S3 `covenant-inbound-email` + receipt rule (`support@`, `press@`) + SNS `covenant-inbound` → the `:ses` ingress. `production.rb` guarded by `:ses`/`:support` credentials; bucket policy grants `cohwall-ses` `GetObject`+`ListBucket` (a permissions boundary blocked the identity-policy route). **Deployed via Kamal to `covenant.verkilo.com`** (subdomain; shared kamal-proxy + Let's Encrypt; bind-mount storage `/var/lib/covenant`; `assume_ssl`/`force_ssl`). Verified end-to-end: magic-link DKIM `d=verkilo.com` + DMARC pass; a real `support@` email opened a Ticket.
- **Gem/SES gotchas fixed**: `aws-actionmailbox-ses` uses the **`:ses`** ingress + **singular** `subscribed_topic`; the S3 receipt action must carry the SNS topic or nothing notifies; **SES rewrites the outbound Message-ID**, so the token-in-Message-ID routing (ADR 0010) can't survive — agent replies/openers now send with `deliver_now`, capture the **SES-assigned** Message-ID, store it on the Reply/Ticket, and thread on that via header matching. Erratum on [[0010-inbound-email-action-mailbox-ses]].
- pages touched: [[ses-migration-runbook]], [[0010-inbound-email-action-mailbox-ses]], [[overview]], [[log]]
- refs: `config/deploy.yml`, `config/environments/production.rb`, `app/mailers/ticket_mailer.rb`, `app/controllers/tickets/replies_controller.rb`
...
- Inbound routing extracts the token straight from `In-Reply-To`/`References` Message-IDs (not plus-addressing — the outbound From stays clean `support@`), falls back to stored-Message-ID matching, then opens a new ticket. `TicketMailer#acknowledgement` is the autoresponder fired on inbound-opened tickets (Basecamp-style 24/5 copy, branded Covenant). Desk is admin-only (agents = `domain_admin`).
- Theme repointed to **Pine** (brand tokens' chroma dropped ~75%, light + both dark blocks). New CSS: `status-pill`, `ticket`, `.comment--outbound`. App menu gains a Tickets card + admin links. SES/AWS prod config left commented (gem: `aws-actionmailbox-ses`, ingress `:ses`).
- Tests: 4 model + mailbox + mailer + integration (26 new); full suite 196 green, RuboCop clean.
- pages touched: [[support-desk-plan]], [[log]]
- refs: db/migrate/2026072207000{1..4}, app/{models,mailers,mailboxes,controllers,policies}, app/assets/stylesheets/{01-tokens,status-pill,ticket,comment}.css

## [2026-07-22] decision | Inbound support email — Action Mailbox + Message-ID token routing (ADR 0010)
- Filed [[0010-inbound-email-action-mailbox-ses]] (accepted). Receive via Action Mailbox `:amazon` ingress (SES receipt rule → SNS → `/rails/action_mailbox/amazon/inbound_emails`). Route by a signed `generates_token_for(:ticket_reply)` token on the stable `Record`, carried in our outbound `Message-ID` (`<reply-<record_id>-<token>@domain>`) and echoed back by the client in `In-Reply-To`/`References` — the 37signals/Help Scout pattern (verified against a real Basecamp support .eml: clean `support@`, no Reply-To, token in the Message-ID), improved with a crypto token. Precedence: token → direct Message-ID match → new ticket. Sender email resolves the Customer only; dedup on Message-ID for idempotent ingest.
- Extends [[0008-email-relay-amazon-ses]] (send-only): adds an inbound SES receipt rule + MX + SNS topic — needs an inbound section in [[ses-migration-runbook]]. Plus-addressed Reply-To rejected as primary (uglier address); sender-email routing rejected (ambiguous with multiple tickets per customer).
- pages touched: [[0010-inbound-email-action-mailbox-ses]] (new), [[support-desk-plan]], [[index]]
- refs: ~/Downloads/2204298019 - Re_ Copy Link doesn't work in Android.eml (Help Scout specimen)

## [2026-07-22] decision | Support desk domain — Customers, Licenses, Tickets, Replies (ADR 0009)
- Filed [[0009-support-desk-customers-licenses-tickets]] (accepted) + [[support-desk-plan]] (self-contained build plan). Additive on the existing spine ([[0006-record-recordable-generic-spine]]/[[0007-versioned-recordables]]), single-tenant, SQLite. `Customer` = plain table (the collector, not a User). `License`/`Ticket`/`Reply` = recordables (not Publishable), immutable → free audit history. Ticket `has_rich_text` the customer's opening email; each later message is a `Reply` (inbound/outbound, nullable creator) threaded under the ticket's record via `parent_id`; `Ticket#replies` mirrors `Record#comments`. Accent theme → Pine (token repoint only); CSS stays CUBE/BEM per [[css-architecture]].
- Open items recorded in the plan: ticket status set, inbound-ticket creator (system user vs nullable), license-key uniqueness among `current`, attachments (phase 2), inbound domain.
- pages touched: [[0009-support-desk-customers-licenses-tickets]] (new), [[support-desk-plan]] (new), [[index]]
- refs: docs/support-desk-plan.md
